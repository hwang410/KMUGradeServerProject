...
def class_user_submit():
    ...
    try:
        submissions = dao.query(Submissions.courseId, Submissions.status, Submissions.submissionCount, Submissions.codeSubmissionDate, \
                                RegisteredCourses.courseName, Problems.problemName)\
    ...
    return render_template('/class_user_submit.html', error=error, ownCourses=ownCourses, submissions=submissions)

@GradeServer.route('/classmaster/cm_manage_problem', methods=['GET', 'POST'])
@login_required
def class_manage_problem():
    error = None
    allProblems = dao.query(Problems).all()
    ownCourses = dao.query(RegisteredCourses).filter_by(courseAdministratorId=session['memberId']).all()
    ownProblems = dao.query(RegisteredProblems.courseId, RegisteredProblems.isAllInputCaseInOneFile, RegisteredProblems.startDateOfSubmission, \
                            RegisteredProblems.endDateOfSubmission, RegisteredProblems.openDate, RegisteredProblems.closeDate, RegisteredProblems.problemId, \
                            RegisteredCourses.courseName, Problems.problemName)\
                        .join(RegisteredCourses, RegisteredCourses.courseId==RegisteredProblems.courseId)\
                        .join(Problems, Problems.problemId==RegisteredProblems.problemId)\
                        .filter(RegisteredCourses.courseAdministratorId==session['memberId']).all()
    ...
    if request.method == 'POST':
        ...
        for form in request.form:
            if 'delete' in form:
                ...
                targetProblem = dao.query(RegisteredProblems).filter(and_(RegisteredProblems.courseId==courseId, RegisteredProblems.problemId==problemId)).first()
                ...
                # actually editTarget is 'id' value of tag. That's why it may have 'Tab' at the last of id to clarify whether it's 'all' tab or any tab of each course.
                ...
                for ownProblem in ownProblems:
                    if ownProblem.courseId==courseId and ownProblem.problemId==int(problemId):
                        ...
                        dao.query(RegisteredProblems).filter(and_(RegisteredProblems.courseId==courseId, RegisteredProblems.problemId==problemId)).update(dict(**kwargs))
                        dao.commit()
        ...
        if isNewProblem:
            ...
            solutionCheckType = dao.query(Problems).filter_by(problemId=problemId).first().solutionCheckType
            newProblem = RegisteredProblems(problemId=problemId, courseId=courseId, solutionCheckType=solutionCheckType, isAllInputCaseInOneFile=isAllInputCaseInOneFile, \
                                       startDateOfSubmission=startDate, endDateOfSubmission=endDate, openDate=openDate, closeDate=closeDate)
    ...
    return render_template('/class_manage_problem.html', error=error, allProblems=allProblems, ownCourses=ownCourses, ownProblems=ownProblems)
...
def class_manage_user():
    ...
    allUsers = dao.query(DepartmentsDetailsOfMembers.memberId, DepartmentsDetailsOfMembers.collegeIndex, DepartmentsDetailsOfMembers.departmentIndex, Members.memberName)\
                    .join(Members, Members.memberId==DepartmentsDetailsOfMembers.memberId).filter_by(authority="User")\
                    .order_by(DepartmentsDetailsOfMembers.memberId).all()
    
    colleges = dao.query(Colleges).all()
    departments = dao.query(Departments).all()
    
    allUsersToData = []
    userIndex = 1
    for index, eachUser in enumerate(allUsers):
        if index==0:
            allUsersToData.append([eachUser.memberId, eachUser.memberName, eachUser.collegeIndex, eachUser.departmentIndex])
        else:
    ...
    for ownCourse in ownCourses:
        try:
            ownUsersOfCourse = dao.query(Registrations.courseId, Registrations.memberId, Members.memberName, Colleges.collegeName, Departments.departmentName)\
                                    .join(Members, Registrations.memberId==Members.memberId)\
                                    .join(DepartmentsDetailsOfMembers, DepartmentsDetailsOfMembers.memberId==Registrations.memberId)\
                                    .join(Colleges, Colleges.collegeIndex==DepartmentsDetailsOfMembers.collegeIndex)\
                                    .join(Departments, Departments.departmentIndex==DepartmentsDetailsOfMembers.departmentIndex)\
                                    .filter(Registrations.courseId==ownCourse.courseId).all()
    ...
    for index, eachUser in enumerate(ownUsers):
        if index==0:
            ownUsersToData.append([eachUser.courseId, eachUser.memberId, eachUser.memberName, eachUser.collegeName, eachUser.departmentName])
        else:
    ...
    if request.method == 'POST':
        ...
        for form in request.form:
            if 'delete' in form:
                ...
                targetUser = dao.query(Registrations).filter(and_(Registrations.courseId==courseId, Registrations.memberId==memberId)).first()
                ...
                try:
                    newMember = Registrations(memberId=newMemberId, courseId=targetCourseId)
                    dao.add(newMember)
                    dao.commit()
                ...
                registeredProblems = dao.query(RegisteredProblems.problemId, RegisteredProblems.courseId, Problems.problemName, RegisteredCourses.courseName)\
                                        .join(Problems, RegisteredProblems.problemId==Problems.problemId).filter(RegisteredProblems.courseId==targetCourseId).all()
    ...
    return render_template('/class_manage_user.html', error=error, ownCourses=ownCourses, ownUsers=ownUsersToData, allUsers=allUsersToData, colleges=colleges, departments=departments)
