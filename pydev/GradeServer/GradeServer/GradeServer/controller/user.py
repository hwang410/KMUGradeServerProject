...
def user_history(memberId, sortCondition, pageNum):
    ...
    try :       
        ...
        sumOfSolvedProblemCount =dao.query (func.count (submissions.c.problemId).label ("sumOfSolvedProblemCount")).\
            ...
            group_by (submissions.c.problemId, submissions.c.courseId).subquery ()
        # 모든 맞춘 횟수
        ...
        try :                           
            ...
            # 차트 정보
            chartSubmissionRecords =dao.query (sumOfSolvedProblemCount, sumOfSubmissionCount, sumOfSolvedCount, sumOfWrongAnswerCount,
                                              sumOfTimeOverCount, sumOfCompileErrorCount, sumOfRunTimeErrorCount, sumOfServerErrorCount).first ()
        except Exception :
            #None Type Exception
            submissionRecords =[]
            chartSubmissionRecords =[]
            
        return render_template('/user_history.html', memberId =memberId,
...
def id_check(select, error =None):
    ...
    if request.method == "POST":
        ...
        if not request.form['password']:
            ...
            try:
                ...
                check = dao.query(Members.password).filter_by(memberId = memberId).first()
    ...
    return render_template('/id_check.html', error=error)
    ...
    try :
        ...
        try :
            memberInformation =dao.query (DepartmentsDetailsOfMembers, Members.memberName, Members.contactNumber, Members.emailAddress, Members.comment,\
                                      Colleges.collegeName, Departments.departmentName)\
                                ...
                                .join(Members, Members.memberId==session['memberId'])\
                                .filter(DepartmentsDetailsOfMembers.memberId ==session['memberId']).all ()
            memberData = []
            loopIndex = 0
